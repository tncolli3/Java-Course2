       ...
       public void actionPerformed(ActionEvent event)
        {
            ...
            if(action==start)//creates a new wave if start is pressed
            {
				wPanel.resume();
			}

			if(action==clear)//gets rid of all waves in that panel
			{
				wPanel.clearPanel();
			}
            ...
            if (action == colorButton)//allows the color to be changed
             {
   ...
   private class DelayListener implements ChangeListener
    {
        public void stateChanged(ChangeEvent event)
         {
			 int newDelay=delay.getValue();
			 wPanel.setDelay(newDelay);
             /***to be completed***/
         }
     ...
     private class WaveWidthListener implements ChangeListener
      {
          public void stateChanged(ChangeEvent event)
           {
			   int newWidth=waveWidth.getValue();
			   wPanel.setWaveWidth(newWidth);
